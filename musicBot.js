...
client.on('message', message => {
	...
	if (msg.substring(0, 1) == '!') {
		...
		if( cmd.length > 1 ){
            ...
            .setColor('GREEN')
            .addBlankField(false)
            .addField('[음악 신청 방법]', '유튜브 URL 을 이용해서 음악 신청 가능. 다음과 같이 입력. \n!add https://www.youtube.com/watch?v=mRWxGCDBRNY 감성 음악 신청 합니다.', false)
            ...
            .addField('[볼륨 조절]', '플레이되고 있는 음악의 보륨을 조절 1 ~ 0 사이의 소수 값을 입력. (내가 신청한 음악만 조절 가능) \n!vol 0.5', false)
						.addField('[MP3 다운로드]', '유튜브 URL 을 이용해서 MP3 파일로 다운로드 가능. 다음과 같이 입력. \n!mp3 https://www.youtube.com/watch?v=mRWxGCDBRNY', false)
					...
					if( client.user.id != message.author.id ) {
						...
						ytdl.getInfo(url, function(error, info) {
							...
							if( length <= 10 ) {
								...
								message.author.send(info.title, new Discord.Attachment(stream, encodeURIComponent(info.title) + '.mp3'));
							}
							else {
